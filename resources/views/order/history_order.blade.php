            ...
            <div class="container my-3">
                ...
                <div class="collapse navbar-collapse" id="navbarNav">
                    <ul class="navbar-nav ms-auto">
                        ...
                        <li class="nav-item">
                            <a class="nav-link text-light me-2" href="{{ route('new.order.form') }}">Tạo đơn hàng mới</a>
                        </li>
                        <li class="nav-item">
                ...
                <div class="collapse d-md-none" id="mobileNav">
                    <ul class="navbar-nav ms-auto">
                        ...
                        <li class="nav-item">
                            <a class="nav-link text-light" href="{{ route('new.order.form') }}">Tạo đơn hàng mới</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link text-light" href="http://127.0.0.1:8000/logout">Đăng xuất</a>
                        </li>
        ...
        <table class="table table-bordered text-center border border-dark table-hover">
            ...
            <tbody class="bg-light" id="orderTableBody">
                @foreach($orders as $order)
                    ...
                    @endphp
                    <tr>
                        <td rowspan="{{ $orderDetails->count() }}">{{ $loop->iteration }}</td>
                        <td rowspan="{{ $orderDetails->count() }}">{{ $order->MaHD }}</td>
                        <td rowspan="{{ $orderDetails->count() }}">{{ $order->TenKhachHang }}</td>
                        <!-- <td rowspan="{{ $orderDetails->count() }}">{{ $order->soDienThoai }}</td> -->
                        <td rowspan="{{ $orderDetails->count() }}">{{ \Carbon\Carbon::parse($order->NgayDatHang)->format('d/m/Y') }}</td>
                        <td rowspan="{{ $orderDetails->count() }}">{{ \Carbon\Carbon::parse($order->GioDatHang)->format('H:i') }}</td>
                        @foreach ($orderDetails as $index => $detail)
                            ...
                            <td>{{ $product ? $product->tensanpham : 'N/A' }}</td>
                            <td>{{ $detail->soLuong }}</td> -->
                            @if ($index === 0)
                                <!-- <td rowspan="{{ $orderDetails->count() }}">{{ $order->PhuongThucThanhToan }}</td> -->
                                <td rowspan="{{ $orderDetails->count() }}">{{ number_format($order->tongtien, 0, ',', '.') }}</td>
                                <!-- <td rowspan="{{ $orderDetails->count() }}">{{ number_format($orderDetails->first()->soTienKhachDua ?? 0, 0, ',', '.') }}</td>
                                <td rowspan="{{ $orderDetails->count() }}">{{ number_format($orderDetails->first()->soTienTraLaiKhach ?? 0, 0, ',', '.') }}</td> -->
                                <td rowspan="{{ $orderDetails->count() }}">
                                    @php
                                ...
                                <td rowspan="{{ $orderDetails->count() }}">
                                    <a href="{{ route('order.detail', ['id' => $order->id]) }}" class="btn btn-info">Xem</a>
                                </td>
                                <td rowspan="{{ $orderDetails->count() }}">
                                    <button class="btn btn-danger delete-btn" data-id="{{ $order->id }}">Xóa</button>
