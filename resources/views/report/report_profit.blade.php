...
<body>
    ...
    <header class="custom-header-bg">
        <nav class="navbar navbar-expand-lg navbar-light">
            <div class="container my-3">   
                <div>
                    <a class="navbar-brand fw-bold fs-2 text-light" href="{{ route('report.report.profit.generate') }}">ANKHANG STORE</a>
                </div>
            </div>
        </nav>
    </header>
    ...
    <div class="container mt-4">
        ...
        <div>
            @if(isset($month))
                <h5 class="text-center mb-4">Trong tháng {{ \Carbon\Carbon::parse($monthYear)->format('m/Y') }}</h5>
            @else
                <h5 class="text-center mb-4">Trong năm {{ \Carbon\Carbon::parse($monthYear)->format('Y') }}</h5>
            @endif
        </div>
        <div class="row text-center">
            <div class="col-6">
                <div>
                    ...
                    <span>{{ number_format($soLuongDonHang, 0, ',', '.') }}</span>
                </div>
                <div>   
                    <label class="for-label">Số lượng sản phẩm:</label>
                    <span>{{ number_format($soLuongSanPham, 0, ',', '.') }}</span>
                </div>
                <div>
                    <label class="for-label">Tổng số tiền thu được:</label>
                    <span>{{ number_format($totalSalesRevenue, 0, ',', '.') }} VND</span>
                </div>
            </div>
            <div class="col-6">
                <div>
                    <label class="for-label">Tổng chi phí nhập hàng:</label>
                    <span>{{ number_format($totalPurchaseCost, 0, ',', '.') }} VND</span>
                </div>
                <div>
                    <label class="for-label">Tổng lợi nhuận:</label>
                    <span>{{ number_format($profit, 0, ',', '.') }} VND</span>
                </div>
                <div>
                    <label class="for-label">Kết luận:</label>
                    <span>{{ $checkProfit }}</span>
                </div>
            </div>
        </div>
            ...
            <tbody>
                @foreach($donHangs as $donHang)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('order.detail', ['id' => $donHang->id]) }}" class="btn btn-info">Xem</a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
